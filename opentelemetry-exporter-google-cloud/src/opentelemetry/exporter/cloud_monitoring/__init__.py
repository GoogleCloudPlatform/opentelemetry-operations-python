...
class CloudMonitoringMetricsExporter(MetricsExporter):
    ...
    def _translate_exemplars(self, exemplars):
        translated_exemplars = []
        for exemplar in exemplars:
            attachments = []
            if exemplar.trace_id:
                # TODO: Pass dropped labels
                # see https://cloud.google.com/monitoring/api/ref_v3/rest/v3/TypedValue#Exemplar,
                # https://github.com/googleapis/google-cloud-dotnet/blob/master/apis/Google.Cloud.Monitoring.V3/Google.Cloud.Monitoring.V3/DroppedLabels.cs
                attachments.append(
                    {
                        "value": SpanContext(
                            span_name="projects/{}/traces/{}/spans/{}".format(
                                self.project_id,
                                get_hexadecimal_trace_id(exemplar.trace_id),
                                get_hexadecimal_span_id(exemplar.span_id),
                            )
            ...
            translated_exemplars.append(
                {
                    ...
                    "attachments": attachments if exemplar.trace_id else [],
                }
            )
            ...
            if isinstance(record.aggregator, HistogramAggregator):
                ...
                point_dict["value"] = {
                    "distribution_value": Distribution(
                        ...
                        exemplars=self._translate_exemplars(
